...
void __time_critical_func(lis3mdl_read)(float *dest, uint i2c_addr, bool mode) {
    ...
    *dest = mode ? INT_N_TO_AX(zval, 16)
                 : INT_N_TO_AX(yval, 16); // atan2f((float)zval, (float)xval);
...
void __time_critical_func(hx_drdy_isr)(uint gpio, uint32_t events) {
    (void)gpio;
    (void)events;
    lis3mdl_read(&_hx_val, I2C_HX_ADDR, true);
}

void __time_critical_func(hy_drdy_isr)(uint gpio, uint32_t events) {
    (void)gpio;
    (void)events;
    lis3mdl_read(&_hy_val, I2C_HY_ADDR, false);
}
...
void phobri_v1_x_3d_core1_init(void) {
    ...
    // gpio_set_irq_enabled_with_callback(STICK_HX_DRDY, GPIO_IRQ_EDGE_RISE,
    // true,
    //                                    &hx_drdy_isr);
    // gpio_set_irq_enabled_with_callback(STICK_HY_DRDY, GPIO_IRQ_EDGE_RISE,
    // true,
    //                                   &hy_drdy_isr);
}
